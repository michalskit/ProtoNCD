...
def split_cluster_acc_v2(y_true, y_pred, num_seen):
    ...
    # it is mapping from true to 
    # most optimal true.
    # thus total_acc:
    # (np.array([ind_map[true] for true in y_true])==y_pred).sum()/y_true.size 
    ind_map = {j: i for i, j in ind}
    # I'd rather want swapped mapping:
    swapped_ind_map = {i: j for i, j in ind}
    ...
    return results, swapped_ind_map
