...
def mixup_data(x: torch.Tensor, y: torch.Tensor, alpha: float = 1.0) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, float]:
    # returns value from beta distribution for alpha=0.5
    # chart: https://en.wikipedia.org/wiki/Beta_distribution#/media/File:Beta_distribution_pdf.svg
    lam = np.random.beta(alpha, alpha) if alpha > 0 else 1.
    # returns random permutation of integers from 0 to x.shape[0] - 1.
