...
def get_transforms(dataset='imagenet', args=None):
    multicrop, num_large_crops, num_small_crops = args.multicrop, args.num_large_crops, args.num_small_crops
    if "augmented_cub" in dataset:

        """preparing images for input into CNN, as if it was pre-trained on ImageNet
        Even though it was pre-trained on iNaturalist"""


        transforms_train_test = T.Compose([
        T.Resize((224, 224)),
        T.ToTensor(),
        T.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225]),
        ])

        # transforms_push = T.Compose([
        # T.Resize((224, 224)),
        # T.ToTensor(),
        # T.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225]),
        # ])

        return (transforms_train_test, transforms_train_test)

    elif "imagenet" or "cub" in dataset.lower():
        dataset = "ImageNet"
