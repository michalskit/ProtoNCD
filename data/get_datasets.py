...
get_dataset_funcs = {
    ...
    'augmented_cub': get_cub_datasets,
...
def get_datasets(dataset, train_transform, test_transform, args):
    ...
    if args.pretrain:
        ...
        train_push_dataset = deepcopy(datasets['train_push_labelled'])
    else:
        train_dataset = MergedDataset(labelled_dataset=deepcopy(datasets['train_labelled']),
                                      unlabelled_dataset=deepcopy(datasets['train_unlabelled']))
        
        train_push_dataset = MergedDataset(labelled_dataset=deepcopy(datasets['train_push_labelled']),
                                      unlabelled_dataset=deepcopy(datasets['train_push_unlabelled']))
    ...
    # TODO: sprawdzić czy tu na pewno powinno być: train_push_unlabelled
    # to mi się wydaje, że zrobiłem w ten sposób ponieważ nie chciałem wrzucać
    # train_unlabelled. train_unlabelled zawiera augmentacje.
    
    # ale czy tu nie ma leeak-u danych???
    # to jest to samo co w przypadku oryginalnego kodu
    unlabelled_train_examples_test = deepcopy(datasets['train_push_unlabelled'])
    unlabelled_train_examples_test.transform = test_transform
    # unlabelled_train_examples_test = datasets["val"]
    return train_dataset, train_push_dataset, test_dataset, unlabelled_train_examples_test, test_seen_dataset


def get_class_splits(args):

    # For FGVC datasets, optionally return bespoke splits
    if args.dataset in ('scars', 'cub', 'augmented_cub', 'aircraft'):
    ...
    elif args.dataset == 'cub' or args.dataset == 'augmented_cub':
